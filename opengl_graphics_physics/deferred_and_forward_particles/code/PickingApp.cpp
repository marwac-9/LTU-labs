...
*/
	...
	void PickingApp::LoadScene2()
	{
		...
		Object* pointLight = SceneGraph::Instance()->addPointLight(false, Vector3(0.f, 10.f, 0.f));
		...
		pSystem->SetTexture(GraphicsStorage::textures["particle2"]->handle);
	...
	void PickingApp::LoadScene3()
	{
		...
		for (int i = 0; i < 500; i++)
		{
			Object* object = SceneGraph::Instance()->addObject("icosphere", SceneGraph::Instance()->generateRandomIntervallVectorCubic(-80, 80));
			RigidBody* body = new RigidBody();
	...
	{
		Object* pointLight = SceneGraph::Instance()->addPointLight(false, currentCamera->GetPosition2()+currentCamera->forward*3.f, Vector3F(1.f, 1.f, 0.f));
		pointLight->node->SetScale(Vector3(10.f, 10.f, 10.f));

		RigidBody* body = new RigidBody();
		pointLight->AddComponent(body, true);
		body->SetCanSleep(false);
		body->ApplyImpulse(currentCamera->forward*4000.f, pointLight->node->GetWorldPosition());
	...
	void PickingApp::SetUpBuffers(int windowWidth, int windowHeight)
	{
		...
		diffuseTexture = new Texture(GL_TEXTURE_2D, 0, GL_RGB, windowWidth, windowHeight, GL_RGB, GL_FLOAT, NULL, GL_COLOR_ATTACHMENT1);
		...
		Texture* materialPropertiesTexture = new Texture(GL_TEXTURE_2D, 0, GL_RGBA32F, windowWidth, windowHeight, GL_RGBA, GL_FLOAT, NULL, GL_COLOR_ATTACHMENT3);
		...
		Texture* finalColor = new Texture(GL_TEXTURE_2D, 0, GL_RGB32F, windowWidth, windowHeight, GL_RGB, GL_FLOAT, NULL, GL_COLOR_ATTACHMENT4);
		finalColor->GenerateBindSpecify();
		finalColor->SetClampingToEdge();
		finalColor->SetLinear();
		Texture* depthTexture = new Texture(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8, windowWidth, windowHeight, GL_DEPTH_COMPONENT, GL_FLOAT, NULL, GL_DEPTH_STENCIL_ATTACHMENT);
		...
		geometryBuffer->RegisterTexture(finalColor);
		...
		Texture* pickingDepth = new Texture(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, windowWidth, windowHeight, GL_DEPTH_COMPONENT, GL_FLOAT, NULL, GL_DEPTH_ATTACHMENT);
