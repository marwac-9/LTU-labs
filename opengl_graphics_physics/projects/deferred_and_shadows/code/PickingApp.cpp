...
*/
	...
	PickingApp::BlurShadowMap()
	{
		...
		for (int i = 0; i < 2; i++){
			...
			GLenum DrawShadowBuffers[] = { GL_COLOR_ATTACHMENT5 };
	...
	void PickingApp::ActivateTextures()
	{
		...
		glBindTexture(GL_TEXTURE_2D, FBOManager::Instance()->shadowMapHandle);
		GLuint shadowMapSamplerSampler = glGetUniformLocation(ShaderManager::Instance()->GetCurrentShaderID(), "shadowMapSampler");
		glUniform1i(shadowMapSamplerSampler, 4);
	}
