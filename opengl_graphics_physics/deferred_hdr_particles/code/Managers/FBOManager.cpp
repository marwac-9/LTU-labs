...
void FBOManager::SetUpDeferredFrameBuffer(int windowWidth, int windowHeight)
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, geoDepthTextureHandle, 0);
...
void FBOManager::SetUpLightAndPostFrameBuffer(int windowWidth, int windowHeight)
{
	...
	// Create the FBO
	glGenFramebuffers(1, &lightAndPostBufferHandle);
	glBindFramebuffer(GL_FRAMEBUFFER, lightAndPostBufferHandle);
...
void FBOManager::BindLightPostBuffer(FrameBufferMode readWrite)
{
	switch (readWrite)
	{
	case read:
		glBindFramebuffer(GL_READ_FRAMEBUFFER, lightAndPostBufferHandle);
		break;
	case draw:
		glBindFramebuffer(GL_DRAW_FRAMEBUFFER, lightAndPostBufferHandle);
		break;
	case readDraw:
		glBindFramebuffer(GL_FRAMEBUFFER, lightAndPostBufferHandle);
		break;
